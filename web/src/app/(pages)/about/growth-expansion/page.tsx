...
export default function GrowthExpansionPage() {
    ...
    return (
        <div className="space-y-16 animate-fade-in-up">
            ...
            <div ref={containerRef} className="relative">
                {/* Vertical Line */}
                <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-1 bg-zinc-100 -translate-x-1/2 hidden md:block overflow-hidden rounded-full">
                    <motion.div
                        className="absolute top-0 left-0 right-0 bg-[#7a0b3a] origin-top h-full"
                        style={{ scaleY }}
                    />
                </div>
            ...
            <section className="bg-[#7a0b3a] rounded-3xl p-12 text-white relative overflow-hidden shadow-2xl">
                ...
                <div className="relative z-10 text-center space-y-6">
                    <h3 className="text-3xl font-serif font-bold">Scaling Excellence</h3>
                    <p className="text-white/80 max-w-2xl mx-auto text-lg font-light">
                        What sets CM College apart is our unwavering commitment to growth that empowers.
                        We don't just expand campus size; we expand the horizons of our students.
                    </p>
...
function MilestoneItem({ milestone, idx }: { milestone: Milestone; idx: number }) {
    ...
    const x = useTransform(scrollYProgress, [0, 0.3], [idx % 2 === 0 ? 50 : -50, 0]);
    const textColor = useTransform(scrollYProgress, [0.4, 0.8], ["#a1a1aa", "#52525b"]); // zinc-400 to zinc-600
    ...
    return (
        ...
        >
            ...
            <div className="absolute left-1/2 -translate-x-1/2 hidden md:flex items-center justify-center z-10">
                <motion.div
                    style={{
                        scale: useTransform(scrollYProgress, [0.4, 0.6], [0.8, 1.1]),
                        boxShadow: useTransform(scrollYProgress, [0.4, 0.6], ["0px 0px 0px rgba(122,11,58,0)", "0px 10px 30px rgba(122,11,58,0.3)"])
                    }}
                    className={`w-12 h-12 rounded-full ${milestone.color} flex items-center justify-center shadow-lg border-4 border-white transition-transform`}
                >
            ...
            <motion.div
                style={{ x }}
                className={`w-full md:w-[45%] ${idx % 2 === 0 ? "md:text-right" : "md:text-left"}`}
            >
            ...
            <div className="w-full md:hidden flex items-center gap-4">
                <motion.div
                    style={{ scale: useTransform(scrollYProgress, [0, 0.3], [0.5, 1]) }}
                    className={`w-10 h-10 rounded-full ${milestone.color} shrink-0 flex items-center justify-center shadow-md`}
                >
                    <milestone.icon className="text-white w-5 h-5" />
                </motion.div>
                <div className="h-px flex-1 bg-zinc-100 relative overflow-hidden">
                    <motion.div
                        className={`absolute inset-0 ${milestone.color} origin-left`}
